		...
		for k,v in json_dict.items():
			...
			if xcounter==0:
				...
				if only_1st not in temp_list_k:
					...
					x=temp_dict[k] - v ## x is the difference between the value in the first file and the second file
					temp_dict[k]=range_conversion(x,float(c),1)
...
def main():
	...
	final_dict=diff_jsons(args.cutoff, list_of_json)

	## writes output file
	## output file have the original name of input jsons in order to be parsable by other scripts
	out_file = open(jsonfiles[0][:-5] + "_vs_" + jsonfiles[1][:-5] + ".json", "w")
